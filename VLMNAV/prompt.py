def construct_action_prompt(goal: str, num_actions: int, turn_around_available: bool):
    ...
    prompt = (
        f"You are an Survival Robot with solar panel attached on top with an RGB image sensor. "
        f"Your task is to navigate to the nearest {goal.upper()}, and get as close to it as possible. "
        f"Use your prior knowledge about where items are typically located in general. "
